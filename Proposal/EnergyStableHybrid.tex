...
and Wilcox, the methods of both Almquist and Friedrich fail to extend
...
%Finally, we also note that these SBP operators do not guarantee strict
...
(Eqn.~\eref{eq:wave1}--\eref{eq:wave2}), in two dimensions and including the
...
reference element $\tilde{\Omega}$. To obtain a version of the governing equations~\eref{eq:wave1}--\eref{eq:wave2}
...
%\end{align}
Applying integration by parts to Eqn.~\eref{eq:wave1_var} to move the spatial derivatives to
...
the per-block semi-discrete expressions for energy dissipation (Eqn.~\eref{eq:sbp_diss} and~\eref{eq:dg_diss})
and a careful definition of the interface quantities themselves,
...
at the interface for the governing equations \eref{eq:wave1}--\eref{eq:wave2} are continuity
...
and $\pmb{v}^{*}$ in the energy dissipation relations (Eqn.~\eref{eq:sbp_diss} for SBP blocks
and Eqn.~\eref{eq:dg_diss} for DG) can be obtained from
\begin{align}
  \label{eqn:pen:con:v}
  \pmb{p}^* - \pmb{p} =\;&
  \pmb{p}^{*} - \pmb{p}^{-} =
...
%\begin{align}\label{eqn:project:noncon}
%  \pmb{\bar{p}}^{\pm} =
%  \pmb{P}_{f2g}^{\pm} {\left( \pmb{S}_{J}^{\pm} \right)}^{1/2} \pmb{p}^{\pm},
%  \qquad
%  \vec{\bar{v}}^{\pm} =
%  \pmb{P}_{f2g}^{\pm} {\left( \pmb{S}_{J}^{\pm} \right)}^{1/2} \pmb{v}^{\pm},
%\end{align}
\begin{align}
...
\pmb{\bar{v}}^{-}$ are defined by Eqn.~\eref{eqn:pen:con:v}--\eref{eqn:pen:con:p}
...
As for discontinuous Galerkin, the numerical flux is similarly given by 
\begin{align}

...
are defined using the NASA 9-coefficient polynomial parameterization \cite{mcbride2002nasa}.
...
Returning to the Navier-Stokes equations \eref{eq:consmass} - \eref{eq:pres}, $N_{sp}$ is the number of species
...
values. In \eref{eq:vandermonde}, $n$ is equal to the order of the integrator,
...
we need to introduce additive terms to \eref{eq:mrab}, which we express
...
\end{align}
In the reacting flow formulation with which we are concerned \eref{eq:consmass} - \eref{eq:pres}, we define
...
a result, one possible definition of the additive terms \eref{eq:mr_reacting} can be given as follows:
\begin{align}
...
so too will our approach to defining the additive terms of \eref{eq:mr_reacting}.
...
\end{align}
While the temperature and pressure right-hand sides are given by \eref{eq:temp} and \eref{eq:pres},
...
With these definitions in mind, we can expand \eref{eq:jac} and express
...
\begin{figure}
\centering
\includegraphics[width=0.9\linewidth,trim=4 4 4 4,clip]{figures/autoignition_drift.png}
...
\end{align}
where $s_{q}$ is the state estimate obtained using an order-$q$ scheme. Based on whether
$\|E\|_{wrms}$ is less than or greater than 1, the timestep
...
\end{tabular}
\captionof{table}{Comparing the laminar flame speeds estimated by Leap-PyJac
